...
impl Plugin for SplinesInterpolatorsPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
          PostUpdate,
          crate::component_tween_system::<TranslationSpline>()
        );
    }
}
...
impl splines::interpolate::Interpolate<f32> for W<Vec3> {
      ...
      fn cubic_bezier(t: f32, a: Self, u: Self, v: Self, b: Self) -> Self {
        ...
        W(a.0 * one_t3 + (u.0 * one_t2 * t + v.0 * one_t * t2) * 3. + b.0 * t2 * t)
      }

      fn cubic_bezier_mirrored(t: f32, a: Self, u: Self, v: Self, b: Self) -> Self {
        <Self as splines::interpolate::Interpolate<f32>>::cubic_bezier(t, a, u, W(b.0 + b.0 - v.0), b)
      }
}
...
impl Interpolator for TranslationSpline {
    ...
    fn interpolate(&self, item: &mut Self::Item, value: f32) {
        let translation = self.0.clamped_sample(value).expect("spline not empty");
